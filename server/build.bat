@echo off
setlocal
echo [WSL] Checking for existing image: ws-server...

:: Check if the image exists in Docker
wsl bash -c "docker images -q ws-server | grep -q ."
if %errorlevel% equ 0 (
    echo [SKIP] Image 'ws-server' already exists.
...
wsl bash -c "cd $(wslpath '%cd%') && docker build -f dockerfile -t ws-server ."
if %errorlevel% neq 0 (
    echo.
